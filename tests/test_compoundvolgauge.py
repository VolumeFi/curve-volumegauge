...
def test_exchange_cdai_to_cusdc(_compoundvolgauge, compoundpool, cDAI, tracker, accounts):
    ...
    tx = _compoundvolgauge.exchange(0, 1, 20 * 10 ** 8, 0, {'from': accounts[0]})
    vgas = tx.gas_used
    print("VGaugeGas : " + str(vgas) + " Unit")
    tx = compoundpool.exchange(0, 1, 20 * 10 ** 8, 0, {'from': accounts[0]})
...
def test_exchange_cusdc_to_cdai(_compoundvolgauge, compoundpool, cUSDC, tracker, accounts):
    last_reward_amount = tracker.rewardAmount()
    tx = _compoundvolgauge.exchange(1, 0, 20 * 10 ** 8, 0, {'from': accounts[0]})
    vgas = tx.gas_used
    print("VGaugeGas : " + str(vgas) + " Unit")
    tx = compoundpool.exchange(1, 0, 20 * 10 ** 8, 0, {'from': accounts[0]})
    print("OriginGas : " + str(tx.gas_used) + " Unit")
    print("IncreasedGas : " + str(vgas - tx.gas_used) + " Unit")
    track_data_size = tracker.trackDataSize(accounts[0])
    track_data = tracker.trackData(accounts[0], track_data_size - 1)
...
def test_exchange_underlying_dai_to_usdc(_compoundvolgauge, compoundpool, DAI, tracker, accounts):
    last_reward_amount = tracker.rewardAmount()
    tx = _compoundvolgauge.exchange_underlying(0, 1, 10 ** 18, 0, {'from': accounts[0]})
    vgas = tx.gas_used
    print("VGaugeGas : " + str(vgas) + " Unit")
    tx = compoundpool.exchange_underlying(0, 1, 10 ** 18, 0, {'from': accounts[0]})
    print("OriginGas : " + str(tx.gas_used) + " Unit")
    print("IncreasedGas : " + str(vgas - tx.gas_used) + " Unit")
    track_data_size = tracker.trackDataSize(accounts[0])
    track_data = tracker.trackData(accounts[0], track_data_size - 1)
...
def test_exchange_underlying_usdc_to_dai(_compoundvolgauge, compoundpool, USDC, tracker, accounts):
    last_reward_amount = tracker.rewardAmount()
    tx = _compoundvolgauge.exchange_underlying(1, 0, 10 ** 6, 0, {'from': accounts[0]})
    vgas = tx.gas_used
    print("VGaugeGas : " + str(vgas) + " Unit")
    tx = compoundpool.exchange_underlying(1, 0, 10 ** 6, 0, {'from': accounts[0]})
    print("OriginGas : " + str(tx.gas_used) + " Unit")
    print("IncreasedGas : " + str(vgas - tx.gas_used) + " Unit")
    track_data_size = tracker.trackDataSize(accounts[0])
    track_data = tracker.trackData(accounts[0], track_data_size - 1)
