...
def test_exchange_cdai_to_cusdc(_compoundvolgauge, cDAI, cUSDC, tracker, accounts):
...
def test_exchange_cusdc_to_cdai(_compoundvolgauge, cUSDC, cDAI, tracker, accounts):
    last_reward_amount = tracker.rewardAmount()
...
def test_exchange_underlying_dai_to_usdc(_compoundvolgauge, DAI, USDC, tracker, accounts):
    last_reward_amount = tracker.rewardAmount()
    _compoundvolgauge.exchange_underlying(0, 1, 10 ** 18, 0, {'from': accounts[0]})
    _compoundvolgauge.exchange_underlying(0, 1, 10 ** 18, 0, {'from': accounts[1]})
    track_data_size = tracker.trackDataSize(accounts[0])
    track_data = tracker.trackData(accounts[0], track_data_size - 1)
    current_reward_amount = tracker.rewardAmount()
...
def test_exchange_underlying_usdc_to_dai(_compoundvolgauge, USDC, DAI, tracker, accounts):
    last_reward_amount = tracker.rewardAmount()
