...
def test_exchange_dai_to_usdc(_threepoolvolgauge, DAI, tracker, accounts):
    for i in range(5):
        last_reward_amount = tracker.rewardAmount()
        _threepoolvolgauge.exchange(0, 1, 10 ** 8, 0, {'from': accounts[0]})
...
def test_exchange_usdc_to_dai(_threepoolvolgauge, USDC, tracker, accounts):
    for i in range(5):
        last_reward_amount = tracker.rewardAmount()
        _threepoolvolgauge.exchange(1, 0, 10 ** 8, 0, {'from': accounts[0]})
        track_data_size = tracker.trackDataSize(accounts[0])
        track_data = tracker.trackData(accounts[0], track_data_size - 1)
        current_reward_amount = tracker.rewardAmount()
...
def test_exchange_dai_to_usdt(_threepoolvolgauge, DAI, tracker, accounts):
    for i in range(5):
        last_reward_amount = tracker.rewardAmount()
        _threepoolvolgauge.exchange(0, 2, 10 ** 8, 0, {'from': accounts[0]})
        track_data_size = tracker.trackDataSize(accounts[0])
        track_data = tracker.trackData(accounts[0], track_data_size - 1)
        current_reward_amount = tracker.rewardAmount()
        last_volume = tracker.lastVolume(DAI)
...
def test_exchange_usdt_to_dai(_threepoolvolgauge, USDT, tracker, accounts):
    for i in range(5):
        last_reward_amount = tracker.rewardAmount()
        _threepoolvolgauge.exchange(2, 0, 10 ** 8, 0, {'from': accounts[0]})
        track_data_size = tracker.trackDataSize(accounts[0])
        track_data = tracker.trackData(accounts[0], track_data_size - 1)
        current_reward_amount = tracker.rewardAmount()
...
def test_exchange_usdc_to_usdt(_threepoolvolgauge, USDC, tracker, accounts):
    for i in range(5):
        last_reward_amount = tracker.rewardAmount()
        _threepoolvolgauge.exchange(1, 2, 10 ** 8, 0, {'from': accounts[0]})
        track_data_size = tracker.trackDataSize(accounts[0])
        track_data = tracker.trackData(accounts[0], track_data_size - 1)
        current_reward_amount = tracker.rewardAmount()
        last_volume = tracker.lastVolume(USDC)
...
def test_exchange_usdt_to_usdc(_threepoolvolgauge, USDT, tracker, accounts):
    for i in range(5):
        last_reward_amount = tracker.rewardAmount()
        _threepoolvolgauge.exchange(2, 1, 10 ** 8, 0, {'from': accounts[0]})
        track_data_size = tracker.trackDataSize(accounts[0])
        track_data = tracker.trackData(accounts[0], track_data_size - 1)
        current_reward_amount = tracker.rewardAmount()
        last_volume = tracker.lastVolume(USDT)
