...
@external
def __init__(_coins: address[N_COINS], _underlying_coins: address[N_COINS], _pool_token: address, _A: uint256, _fee: uint256):
    """
...
def remove_liquidity(_amount: uint256, min_amounts: uint256[N_COINS]):
    ...
    for i in range(N_COINS):
        ...
        if tethered[i] and not use_lending[i]:
            ...
            cERC20(self.coins[i]).transfer(msg.sender, value)
...
def remove_liquidity_imbalance(amounts: uint256[N_COINS], max_burn_amount: uint256):
    ...
    for i in range(N_COINS):
        if tethered[i] and not use_lending[i]:
            ...
            cERC20(self.coins[i]).transfer(msg.sender, amounts[i])
...
def apply_transfer_ownership():
    assert msg.sender == self.owner
    assert block.timestamp >= self.transfer_ownership_deadline and self.transfer_ownership_deadline > 0
